function loadtable(){
    const xhttp = new XMLHttpRequest();//new 
    xhttp.open("GET"," https://www.mecallapi.com/api/users");//link API
    xhttp.send();//เรียก
    xhttp.onreadystatechange = function(){
        if(this.readyState == 4 && this.status == 200){//200 = status พร้อมของ API
            console.log(this.responseText);
            var trHTML ='';
            const objs = JSON.parse(this.responseText);// เปลียนจาก text > obj
            for(let object of objs ){
                trHTML += '<tr>';
                trHTML += '<td>' +object['id'] + '</td>';
                trHTML += '<td><img width="50px" src="'+object['avatar']+'"class="avatar">'+'</td>';//img
                trHTML += '<td>' +object['fname'] + '</td>';
                trHTML += '<td>' +object['lname'] + '</td>';
                trHTML += '<td>' +object['UserName'] + '</td>';
                trHTML += '<td><button type="button" onclick="showUserEditBox('+object['id']+')" class="btn btn-success">Edit</button></td>';
                trHTML += '<td><button type="button" onclick="userDelete('+object['id']+')" class="btn btn-danger">Del</button></td>'
                trHTML += '</tr>';
            }
            document.getElementById("mytable").innerHTML = trHTML;//คือการนำ trH.. ไปเเทนที่ในตาราง Frond
        }
    }
}
loadtable();
